...
public class GifSequenceWriter {
	 ...
	 * > 1) { // grab the output image type from the first image in the sequence
	 ...
	 * between frames, which loops continuously GifSequenceWriter writer = new
	 * GifSequenceWriter(output, firstImage.getType(), 1, false);
	 * 
	 * // write out the first image to our sequence...
	 * writer.writeToSequence(firstImage); for(int i=1; i<args.length-1; i++) {
	 * BufferedImage nextImage = ImageIO.read(new File(args[i]));
	 * writer.writeToSequence(nextImage); }
	 * 
	 * writer.close(); output.close(); } else { System.out.println(
